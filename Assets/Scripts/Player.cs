...
public class Player : NetworkBehaviour {
	...
	void RegisterPreMove(Vector2Int from, Vector2Int to) {
		PreMove p = new PreMove { from = from, to = to };
		sbyte castlingResult = CheckForCastling(from, to);
		if (castlingResult != 0) {
			if (castlingResult > 0) {
				p.action = PreMoveSpecialAction.CastleK;
			} else {
				p.action = PreMoveSpecialAction.CastleQ;
			}
		}
		preMoves.Add(p);
	}
	...
	//TODO: Promotion execution on premove execution
